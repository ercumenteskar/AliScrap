...
namespace My
{
  ...
  // MyFunctioNs
  {
    ...
    public static SqlConnection GetConnection(string ConnectionName = "")
    {
      ...
      {
        connstr = "Server=.; Database=Ali; uid=sa; pwd=e;pooling=true; connection lifetime=10; connection timeout=0; packet size=1024;";
      }
  ...
  {
    public static string UppercaseFirst(this string s, bool forEveryWord = true)
    {
      if (string.IsNullOrEmpty(s))
        return string.Empty;
      else if (forEveryWord)
      {
        String tmpstr = "";
        foreach (var item in s.Split(' '))
          tmpstr += " " + char.ToUpper(item[0]) + item.Substring(1).ToLower();
